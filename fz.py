...
#version 1.0
#Credits: based on the first implemented zzCrack
...
class main():
    ...
    def memsafe_generate_bruteforce_list(self, charset, min_lenght, max_lenght):
    ...
    def bruteforce_crack(self, tries, brutelist, z, stream, charset, min_lenght, max_lenght):
        content = z.namelist()
        for word in self.memsafe_generate_bruteforce_list(charset, min_lenght, max_lenght):
            try:
                if self.done == True:
                    confirm = input("\nAre you sure you want to stop (y/n) > ").lower()
                    if confirm == "y": return
    ...
    def _args(self):
        ...
        if options.wordlist != None:
            ...
            min_letters_inp = options.minlenght
            if min_letters_inp == None:
                print("No MinLenght Given")
            try:
                min_letters = int(min_letters_inp)
                if min_letters < 0:
                    print("The given MinLenght is not a vaild Positive number")
                    sys.exit(1)
            except:
                print("The given MinLenght is not a vaild Integer number")
                sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_option("--max-lenght", dest="maxlenght",
                    help="The max lenght for the bruteforce passwords (Required for Bruteforce)", metavar=" ")
    parser.add_option("--min-lenght", dest="minlenght",
                    help="The min lenght for the bruteforce passwords (Required for Bruteforce)", metavar=" ")
